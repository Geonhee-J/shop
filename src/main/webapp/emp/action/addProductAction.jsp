...
<%
    ...
    String empId = (String)(sessionArea.get("empId"));
    
    int result = ProductDAO.insertProduct(category, empId, productTitle, productImg, productContent, productAmount, productPrice, response);
    
    if (result == 1) {
        response.sendRedirect("/shop/emp/productList.jsp");
    } else {
        String errMsg = URLEncoder.encode("잘못된 접근입니다.", "utf-8");
        response.sendRedirect("/shop/emp/addProductForm.jsp?errMsg=" + errMsg);
        return;
    }
%>